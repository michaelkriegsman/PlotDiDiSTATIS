...
Plot_HiDiSTATIS_FiCd <- function(res_HiDiSTATIS, axes = c(1,2), i = 1, d = 1, Flip_axis1 = FALSE, Flip_axis2 = FALSE, dev.new = TRUE){
  ...
  which_tables_Cd <- which(res_HiDiSTATIS$input$DESIGN_tables$mat[,d]==1)

  constraints <- minmaxHelper(mat1 = res_HiDiSTATIS$res_GrandComp$eig$F[,axes] %*% Flip_mat,
                              mat2 = t(res_HiDiSTATIS$res_GrandComp$ProjCP$F[i, axes, which_tables_Cd]) %*% Flip_mat)
  ...
  prettyPlot(res_HiDiSTATIS$res_GrandComp$eig$F[,axes] %*% Flip_mat,
             ...
             main = paste0("HiDiSTATIS: Stimuli are Hierarchically Barycentric \n i = ", i, ", d = ", d))

  #For a single stimulus, Plot F.(D), and connect the lines
  Segments_from_to(From = res_HiDiSTATIS$res_GrandComp$eig$F[i,axes] %*% Flip_mat,
                   To = t(res_HiDiSTATIS$res_GrandComp$ProjGroup$F[i,axes,]) %*% Flip_mat,
                   XandY = axes,
                   col = add.alpha(res_HiDiSTATIS$input$DESIGN_tables$colors_D, alpha),
                   lwd = 2)


  prettyPlot(t(res_HiDiSTATIS$res_GrandComp$ProjGroup$F[i,axes,]) %*% Flip_mat,
             col = add.alpha(res_HiDiSTATIS$input$DESIGN_tables$colors_D, alpha),
             display_names = F,
             cex = 1.75, pch=16,
  ...
  Segments_from_to(From = res_HiDiSTATIS$res_GrandComp$ProjGroup$F[i,axes,d] %*% Flip_mat,
                   To = t(res_HiDiSTATIS$res_GrandComp$ProjCP$F[i, axes, which_tables_Cd]) %*% Flip_mat,
                   XandY = axes,
                   col = add.alpha(res_HiDiSTATIS$input$DESIGN_tables$colors_D[d], alpha),
                   lwd = 3)

  prettyPlot(t(res_HiDiSTATIS$res_GrandComp$ProjCP$F[i, axes, which_tables_Cd]) %*% Flip_mat,
             col = add.alpha(res_HiDiSTATIS$input$DESIGN_tables$colors_D[d], alpha),
             display_names = F,
             cex = 2.25, pch=16,
             dev.new = F, new.plot = F)
